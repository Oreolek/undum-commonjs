...
var isInView = function (element) {
    ...
    var scrollMax = viewportTop + ($(window).height() * 0.9);
    var scrollMin = viewportTop + ($(window).height() * 0.1);

    return (elementTop > scrollMin) && (elementBottom < scrollMax);
};

/* Makes sure the element is visible, by scrolling down if necessary. */
var putInView = function (output) {
    if (isInView(output)) {
        // The element is already in the viewport; do nothing.
        scrollStack = [];
        return;
    }
